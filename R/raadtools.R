...
##' @details \code{xylim} is expected to be consistent with \code{lon180}
...
readsst <- function(date = as.Date("1981-09-01"), time.resolution = "daily", varname = c("sst", "anom", "err", "ice"),
    ...
    for (i in seq_len(nfiles)) {
        ...
        r0[r0 < -2] <- NA
        r[[i]] <- r0
        if (verbose & i > 1 & (i %% (max(c(1, nfiles %/% 20L)))) == 0L) .progressreport(i, nfiles)
    }
...
readcurr <- function(date = as.Date("1999-11-24"),
    ...
    for (i in seq_len(nfiles - 1)) {
        ...
        r <- setValues(r, values(rasterfun(r1, r2)), layer = i)
          if (verbose & i > 1 & i %% (nfiles %/% 20L) == 0L) .progressreport(i, nfiles)
    }
...
readice <- function(date = as.Date("1978-11-01"),
    ...
    for (ifile in seq_along(findex)) {
      ...
      if (verbose & ifile > 1 & ifile %% (nfiles %/% 20L) == 0L) .progressreport(ifile, nfiles)
  }

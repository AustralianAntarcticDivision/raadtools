...
[Sea ice concentration](https://nsidc.org/cryosphere/seaice/data/terminology.html) describes the relative amount of area covered by ice. In raadtools there are different resolutions available and each covers the entire ice-affected area of the Southern Ocean. Each read function returns sea ice concentration as a *fraction*, a value from 0 to 1, whatever the product stores on disk. 

The products are

- **NSIDC:**           25km sea ice concentration, available since October 1978 (every other day until August 1987, then daily), from the NOAA/NSIDC Climate Data Record (the default) or NSIDC-0051 
...
NSIDC-0051  | `readice(product = "nsidc")` | `icefiles(product = "nsidc")`
...
`readice()` reads the Climate Data Record by default, through `read_nsidc_cdr_daily()`. `product = "nsidc"` reads NSIDC-0051 instead, and `read_nsidc_ice_daily()` reads it directly. 
...
Not every day is available in every case. Each function when given a date (or date-time) value will find the latest available data at or before that time, within a certain tolerance (a day and a half for daily data). This means that sometimes we would obtain the same layer for slightly different inputs (i.e. 2 consecutive days may not be differentiated, and the repeated layer is dropped with a warning). This is a convenience for casual usage. For more rigorous use we should obtain each set of files directly and use the available dates explicitly. 
...
These can be regridded or aggregated temporarily in any way that's required using spatial tools in R. Every read function returns a terra `SpatRaster`, so `terra::project()`, `terra::resample()` and `terra::aggregate()` apply directly. 
...
xy <- project(pt, "OGC:CRS84", crs(ice))
buf <- 640000
ex <- ext(xy[1] + c(-0.5, 1) * 1 * buf * 1.852,

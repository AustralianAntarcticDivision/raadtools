...
plotcurrents <- function(date = as.Date("1999-11-24"), ext = NULL, scale = 2000) {
     ...
     x <- crop(x, ext)
     crds <- coordinates(x)
     plot(sqrt(x[["U"]]^2 + x[["V"]]^2), col = sst.pal(25, alpha = 0.7))
     x1 <- crds[,1]
     y1 <- crds[,2]
     x2 <- crds[,1] + values(x[["U"]]) * scale
     y2 <- crds[,2] + values(x[["V"]]) * scale
...
dts <- seq(as.Date("1995-10-01"), length = 30, by = "1 week")
library(animation)
ani.start()
for (i in seq_along(dts)) plotcurrents(dts[i], ext = xylim, scale = 2500)
ani.stop()

...
The actual data sources are understood at a provider level by the tools bowerbird and blueant. 

## Data synchronization

Bowerbird and blueant provide the real value behind raadtools. This is a set of tools that automatically scan online sources, compare checksums to the current data library, and download any sources that are not yet available. The configurations include exclusion and inclusion filters, recursive source scanning, ability to unzip or otherwise unpack sources on download, how to navigate various APIs and virtualized web source links. These configurations look a lot more like recent systems such as STAC and will easily integrate with those. 

...
The package is an R library of functions that handle user queries for reading variables as a function of time and space. There is a mix of  *time series*, *time-depth*, and *time-static* **variables** of various types within datasets. Function design is focussed on variables, with options for reading subsets in time and space. Functions use the pattern `read[_x]` meaning to read a slice from variable `[x]`, with further optional user-supplied arguments. 

The first argument is a (vector of) date-time/s, and the second an extent in space. If no options are given, functions default to returning the entire spatial extent of the latest-available time step.  Each function has a few other standard arguments and some that are variable-specific, we mention some of these details in example below. 
...
When raadtools was born it was clear that time series grid subsetting had these three main modes, 1) read a spatial subset of a given time 2) read a spatio-temporal subset, and 3) look up values for points or lines at discrete or continuous time/s. The raster package provide 1, and 2 without any further infrastructure. (Things have moved on a lot since then in R and other software).

...
allocfile <- function(x, fname, nl = 1) {
  tst <- hdr(x, fname)
  if (nl > 1) {
    txt <- readLines(fname)
    ln <- grep("nbands", txt)
  ...
  dsize <- c(FLT4S = 4)[x@file@datanotation]
  grifile <- file(gsub(".grd$", ".gri", fname), open = "wb")
  seek(grifile, nrow(x) * ncol(x) * dsize - 1L)
  writeBin(raw(1L), grifile)
  close(grifile)
  fname
}


hdr <-  function(x, filename) {
  
  thefile <- file(filename, "w")  # open an txt file connectionis
cat("[general]", "\n", file = thefile)

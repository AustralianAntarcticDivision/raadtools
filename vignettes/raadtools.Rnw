...
without any arguments at all, with a vector of date strings (or vector
of POSIXct or Date objects) to return a single or multiple layer
raster object. The function \textbf{readtopo} will return one of
several available data sets
(\Sexpr{as.character(formals(readtopo)[["topo"]])[-1]}). Some data
sets are inherently multi-layered for a single time step, such as wind
and current vectors and so these both have optional arguments
\textbf{dironly} and \textbf{magonly} to convert the internal
\textbf{U} and \textbf{V} vector components to directions or
...
aurora$sst <- extract(readsst, aurora[,1:3])
aurora$chla <- extract(readchla, aurora[,1:3])

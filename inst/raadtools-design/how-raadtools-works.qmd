...
The raadtools package has access to large data library of general remote sensing and environmental data.
...
This catalogue is grepped by each read function, to get its correct set of files, that is how the specific file sets above are created.  

Then the read(date, ) hones in on the right set of files and we are squarely into "R spatial" context, with raadtools having no further role. 

### The stack

Behind raadtools is the 'raster' package, this is now a legacy raster datacube facility in R. It is still supported, and we have not moved to its successors. What we have done is use GDAL more directly when ...
...
This blogpost works through setting up the file system from scratch. 
